...
fn to_trace(input: &str) -> Vec<(char, i64)> {
    input.split(",")
        .map(|dir| {
            let dir_char = dir.chars().next().unwrap();
            let size: i64 = dir[1..].parse().unwrap();

            (dir_char, size)
        }).collect()
}

fn trace_to_positions(trace: Vec<(char, i64)>) -> Vec<Vec2> {
    let mut current_position = (0, 0);

    trace.into_iter()
        .map(|dir| {
            let current_vec = match dir.0 {
                'U' => (0, 1),
                'R' => (1, 0),
                'D' => (0, -1),
                'L' => (-1, 0),
            ...
            (0..dir.1).map(|i| {
                let next_pos = add(current_position, current_vec);
...
fn main1() {
    ...
    let position_set_1: HashSet<Vec2> = trace_to_positions(to_trace(input1)).into_iter().collect();
    let position_set_2: HashSet<Vec2> = trace_to_positions(to_trace(input2)).into_iter().collect();

    let mut positions_1 = trace_to_positions(to_trace(input1));
    positions_1.insert(0, (0, 0));

    let mut positions_2 = trace_to_positions(to_trace(input2));
    positions_2.insert(0, (0, 0));

    let mut smallest = None;
    let mut smallest_dist = std::i64::MAX;
    for intersecting_position in position_set_1.intersection(&position_set_2) {
        let dist_1 = positions_1.iter().position(|p| p == intersecting_position).unwrap() as i64;
        let dist_2 = positions_2.iter().position(|p| p == intersecting_position).unwrap() as i64;
        let total = dist_1 + dist_2;
        if smallest.is_none() || total < smallest_dist {
            smallest = Some(intersecting_position);
            smallest_dist = total;
        }
    }
...
fn main2() {
    ...
    for intersecting_position in position_set_1.intersection(&position_set_2) {
        if smallest.is_none() {
            ...
            let other_intersecting_position = smallest.unwrap();
            if manhattan((0, 0), *intersecting_position) < manhattan((0, 0), *smallest.unwrap()) {
                smallest = Some(intersecting_position);
            }
            dbg!(&smallest);
        }
    ...
    dbg!(&smallest);
    dbg!(manhattan((0, 0), *smallest.unwrap()));
}
