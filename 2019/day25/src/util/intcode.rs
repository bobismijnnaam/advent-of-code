...
//        dbg!(instr);
    ...
    pub fn resume_inputs(&mut self, input: Vec<i64>) {
        assert!(self.state == IntcodeState::WaitingForInput || self.state == IntcodeState::Running);
    ...
    pub fn resume_string(&mut self, input: &str) {
        for c in input.chars() {
            self.input.push(c as u8 as i64);
        }
        self.state = IntcodeState::Running;
        self.start();
    }
